...
const MockupColorPicker = ({ chosenColor, setChosenColor, colorList, topColorPicker, bottomColorPicker, filterType }) => {
  ...
  return (
    <div className="mockup-color-picker">
        ...
        style={{ backgroundColor: chosenColor.filtered }} onClick={() => setcolorsVisibility(!colorsVisibility)} value="change color">
          <ColorizeIcon style={{color : getBlackOrWhiteAsBestContrast(chosenColor, filterType).filtered}}></ColorizeIcon>
        ...
        <ul className="mockup-color-picker-list" style={{ display: colorsVisibility ?  "flex" : "none" }}> 
            {Object.values(colorList).map((color, index) => (
              <li className="color-picker-list-item-wrap" key={"color-picker-list-item-wrap" + index}>
                    <button onClick={() => setChosenColor(color)} className="colorBlock" title={color.original} style={{ backgroundColor: color.filtered }}></button>
                </li>
            ))}
        </ul>
    </div>
